<!-- SIDEBAR -->
<div class="app-header__logo">
        <div class="logo-src"></div>
        <div class="header__pane ml-auto">
            <div>
                <button type="button" class="hamburger close-sidebar-btn hamburger--elastic" data-class="closed-sidebar">
                    <span class="hamburger-box">
                        <span class="hamburger-inner"></span>
                    </span>
                </button>
            </div>
        </div>
    </div>
    <div class="app-header__mobile-menu">
        <div>
            <button type="button" class="hamburger hamburger--elastic mobile-toggle-nav">
                <span class="hamburger-box">
                    <span class="hamburger-inner"></span>
                </span>
            </button>
        </div>
    </div>
    <div class="app-header__menu">
        <span>
            <button type="button" class="btn-icon btn-icon-only btn btn-primary btn-sm mobile-toggle-header-nav">
                <span class="btn-icon-wrapper">
                    <i class="fa fa-ellipsis-v fa-w-6"></i>
                </span>
            </button>
        </span>
    </div>    
    <div class="scrollbar-sidebar">
        <div class="app-sidebar__inner">
                <ul class="vertical-nav-menu vertical-nav-menu-m">
                    <li class="app-sidebar__heading app-sidebar__heading-m">Dashboards</li>
                        <li>
                            <a href="/dash" class="">
                                <i class="metismenu-icon pe-7s-timer"></i>
                                Main Dashboard
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <i class="metismenu-icon pe-7s-graph3"></i>
                                    Other DashBoards
                                <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                            </a>
                            <ul>
                                <li>
                                    <a href="/sites_dash">
                                        <i class="metismenu-icon"></i>
                                        Sites Dashboard
                                    </a>
                                </li>
                                <li>
                                    <a href="/alerts_dash">
                                        <i class="metismenu-icon"></i>
                                        Alerts Dashboard
                                    </a>
                                </li>
                            </ul>
                        </li>
                    <li class="app-sidebar__heading app-sidebar__heading-m">Sites</li>
                        <li>
                            <a href="#">
                                <i class="metismenu-icon pe-7s-network"></i>
                                Site Management
                                <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                            </a>
                            <ul>
                                <li>
                                    <a href="/cells">
                                        <i class="metismenu-icon"></i>
                                        Cells List
                                    </a>
                                </li>
                                <li>
                                    <a href="/sites">
                                        <i class="metismenu-icon"></i>
                                        Sites List
                                    </a>
                                </li>
                                <li>
                                    <a href="#">
                                        <i class="metismenu-icon">
                                        </i>Site Installations
                                    </a>
                                </li>
                                <li>
                                    <a href="#">
                                        <i class="metismenu-icon">
                                        </i>Site Optimizations
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="#">
                                <i class="metismenu-icon pe-7s-display1"></i>
                                    Site Reports
                                <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                            </a>
                            <ul>
                                <li>
                                    <a href="#">
                                        <i class="metismenu-icon"></i>
                                        Installation Reports
                                    </a>
                                </li>
                                <li>
                                    <a href="#">
                                        <i class="metismenu-icon"></i>
                                        Test Reports
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li class="app-sidebar__heading app-sidebar__heading-m">Alerts</li>
                        <li>
                            <a href="#">
                                <i class="metismenu-icon pe-7s-attention"></i>
                                View Alerts
                                <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                            </a>
                            <ul>
                                <li>
                                    <a href="#">
                                        <i class="metismenu-icon"></i>
                                        Region One
                                    </a>
                                </li>
                                <li>
                                    <a href="#">
                                        <i class="metismenu-icon"></i>
                                        Region Two
                                    </a>
                                </li>
                                <li>
                                    <a href="#">
                                        <i class="metismenu-icon"></i>
                                        Region Three
                                    </a>
                                </li>
                                <li>
                                    <a href="#">
                                        <i class="metismenu-icon"></i>
                                        Region Four
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                                <a href="#">
                                    <i class="metismenu-icon pe-7s-battery"></i>
                                    Alert Thresholds
                                    <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                                </a>
                                <ul>
                                    <li>
                                        <a href="#">
                                            <i class="metismenu-icon"></i>
                                            Azimuth Thresholds
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <i class="metismenu-icon"></i>
                                            Tilt Thresholds
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <i class="metismenu-icon"></i>
                                            Roll Thresholds
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <i class="metismenu-icon"></i>
                                            Battery Thresholds
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <i class="metismenu-icon"></i>
                                            Communcation Timeouts
                                        </a>
                                    </li>
                                </ul>
                            </li>                        
                        
                        <li class="app-sidebar__heading app-sidebar__heading-m">System</li>
                        <li>
                            <a href="#">
                                <i class="metismenu-icon pe-7s-key"></i>
                                Authentication
                                <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                            </a>
                            <ul>
                                <li>
                                    <a href="/roles">
                                        <i class="metismenu-icon pe-7s-key">
                                        </i>Roles
                                    </a>
                                </li>
                                <li>
                                    <a href="/users">
                                        <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                                        Users
                                    </a>
                                </li>
                                
                                <li>
                                    <a href="/contractors">
                                        <i class="metismenu-icon">
                                        </i>Contractors
                                    </a>
                                </li>
                                <li>
                                    <a href="/teams">
                                        <i class="metismenu-icon">
                                        </i>Teams
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                                <a href="#">
                                    <i class="metismenu-icon pe-7s-display2"></i>
                                    System Reports
                                    <i class="metismenu-state-icon pe-7s-angle-down caret-left"></i>
                                </a>
                                <ul>
                                    <li>
                                        <a href="#">
                                            <i class="metismenu-icon">
                                            </i>User Reports
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <i class="metismenu-icon">
                                            </i>Authentication Reports
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <i class="metismenu-icon">
                                            </i>System Logs
                                        </a>
                                    </li>
                                </ul>
                            </li>
                        
                </ul>
        </div>
    </div>
<!-- /SIDEBAR -->